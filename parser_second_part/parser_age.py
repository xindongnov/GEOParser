...
GSM_Element_names = ["Title","Accession","Characteristics",
                     ...
                     "Supplementary-Data","Treatment-Protocol",'Extract-Protocol',"Data-Processing"]
...
def find_all_meta(folder_path,file,key_list):
    if file.startswith('GSM') and file.endswith('.xml'):
                ...
                Organism_value = GSM_meta_dict.get('GSM_Organism',None)
                #match_key,key_bank,key_word=find_key_words(GSM_meta_dict,word_bank_name_list,word_bank_list)
                
                return gsm,char_key,char,Organism_value#match_key,key_bank,key_word,Organism_value
...
def main():
    ...
    parser.add_argument('-r', '--root_folder', type=str,help='path to dirs of GSE', default='/fse/DC/sc_geo_gse')
    ...
    csv_file_name = "age_"+ args.all_meta_file_path
    no_meta_file_name = "age_"+ args.no_meta_file_path
    error_file_name  = "age_"+ args.error_file_path
...
##char_key_list
    ...
    Age_key_list = ['age', 'time', 'day', 'week','year','month']
    #age_bank =['age', 'time', 'day', 'week','year','month']
    ...
    if save_path == '':
        if not os.path.exists('age'):
            os.makedirs("age")
            csv_file_name_path = os.path.join("age", csv_file_name)
            no_meta_file_name_path = os.path.join("age", no_meta_file_name)
            error_file_name_path = os.path.join("age", error_file_name)
    else:
        if not os.path.exists(os.path.join(save_path,"age")):
            os.makedirs(os.path.join(save_path,"age"))
        csv_file_name_path = os.path.join(save_path,"age", csv_file_name)
        no_meta_file_name_path =os.path.join(save_path,"age",  no_meta_file_name)
        error_file_name_path = os.path.join(save_path,"age",  error_file_name)
    if not os.path.isfile(csv_file_name_path):
        with open(csv_file_name_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['gse', 'gsm','Species','char_age_key','char_age'
                                                    ] 
                ...
                try:
                    ...
                    if gse in gse_list:
                        ...
                        with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        fieldnames = ['gse', 'gsm', 'Species','char_age_key',
                                    'char_age']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gse, "Species":'',
                                                                                        'char_age_key':'','char_age':''
                                                                                        })
                        if not only_need and len(files) > 1:
                            for file in files:
                                if file.startswith('GSM') and file.endswith('.xml'):
                                    gsm,char_age_key,char_age,Species = find_all_meta(folder_path,file,Age_key_list)
                                    #if char_age_key != '' or char_age != '':
                                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                            fieldnames = ['gse', 'gsm', 'Species','char_age_key',
                                        'char_age']                            
                                            writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                            writer.writerow({'gse': gse, 'gsm': gsm, "Species":Species,
                                                                                            'char_age_key':char_age_key,'char_age':char_age
                                                                                            })
                    ...
                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                    ...
                                    'char_age']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gsm, "Species":'',
                                                                                        'char_age_key':'','char_age':''
                                                                                        })
            else:
                try:
