...
def main():
    ...
    csv_file_name = "disease_"+ args.all_meta_file_path
    no_meta_file_name = "disease_"+ args.no_meta_file_path
    error_file_name  = "disease_"+ args.error_file_path
    root_folder = args.root_folder
    existed_GSE = args.existed_GSE
    only_need = args.only
    save_path = args.save_path
##char_key_list
    Disease_key_list = ['disease']
    disease_df_type =pd.read_csv('./word_bank/DiseaseType_dictionary_1-multi.tsv',sep='\t')
    disease_bank = disease_df_type['Disease'].to_list()
    ...
    if save_path == '':
        if not os.path.exists('disease'):
            os.makedirs("disease")
            csv_file_name_path = os.path.join("disease", csv_file_name)
            error_file_name_path = os.path.join("disease", error_file_name)
    else:
        if not os.path.exists(os.path.join(save_path,"disease")):
            os.makedirs(os.path.join(save_path,"disease"))
        csv_file_name_path = os.path.join(save_path,"disease", csv_file_name)
        error_file_name_path = os.path.join(save_path,"disease",  error_file_name)
    if not os.path.isfile(csv_file_name_path):
        with open(csv_file_name_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['gse', 'gsm','char_disease_key','char_disease',
                                                    "disease_match_key",'disease_key_bank','disease_key_word'] 
            ...
            if existed_GSE_choose :
                try:
                    ...
                    if  gse in gse_list:
                        ...
                        GSE_match_key,GSE_key_bank,GSE_key_word=find_key_words(GSE_meta_dict,["disease_bank"],[disease_bank])
                        with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        fieldnames = ['gse', 'gsm', 'char_disease_key',
                                    'char_disease',"disease_match_key",'disease_key_bank','disease_key_word']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gse, 
                                                                                        'char_disease_key':'','char_disease':'',
                                                                                        'disease_match_key':GSE_match_key,'disease_key_bank':GSE_key_bank,'disease_key_word':GSE_key_word
                                                                                        })
                        print("{} is starting ".format(gse))
                        if not only_need and len(files) > 1:
                            for file in files:
                                if file.startswith('GSM') and file.endswith('.xml'):
                                    gsm,char_disease_key,char_disease,disease_match_key,disease_key_bank,disease_key_word = find_all_meta(folder_path,file,Disease_key_list,["disease_bank"],[disease_bank])
                                    #if char_disease_key != '' or char_disease != '' or disease_match_key != [] or disease_key_bank != [] or disease_key_word != []:
                                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                            fieldnames = ['gse', 'gsm', 'char_disease_key',
                                        'char_disease',"disease_match_key",'disease_key_bank','disease_key_word']                            
                                            writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                            writer.writerow({'gse': gse, 'gsm': gsm, 
                                                                                            'char_disease_key':char_disease_key,'char_disease':char_disease,
                                                                                            'disease_match_key':disease_match_key,'disease_key_bank':disease_key_bank,'disease_key_word':disease_key_word
                                                                                            })
