...
def main():
    ...
    csv_file_name = "tissue_"+ args.all_meta_file_path
    no_meta_file_name = "tissue_"+ args.no_meta_file_path
    error_file_name  = "tissue_"+ args.error_file_path
    root_folder = args.root_folder
    existed_GSE = args.existed_GSE
    only_need = args.only
    save_path = args.save_path
##char_key_list
    Tissue_key_list = ["tissue"]
    ##tissue
    with open('./word_bank/cancer_cellline.txt','r') as c:
        cancer_celllines_bank=[cancer.strip() for cancer in c]
    with open('./word_bank/tumor_tissue_new.txt','r') as tt:
        tumor_tissues_bank=[word.strip() for word in tt]
    with open('./word_bank/tissue_list_new.txt','r') as t:
        tissue_list_bank=[tissue.strip() for tissue in t]
    ...
    if save_path =='':
        if not os.path.exists('tissue'):
            os.makedirs("tissue")
            csv_file_name_path = os.path.join("tissue", csv_file_name)
            # no_meta_file_name_path = os.path.join("tissue", no_meta_file_name)
            error_file_name_path = os.path.join("tissue", error_file_name)
    else:
        if not os.path.exists(os.path.join(save_path,"tissue")):
            os.makedirs(os.path.join(save_path,"tissue"))
        csv_file_name_path = os.path.join(save_path,"tissue", csv_file_name)
        # no_meta_file_name_path =os.path.join(save_path,"tissue",  no_meta_file_name)
        error_file_name_path = os.path.join(save_path,"tissue",  error_file_name)
    if not os.path.isfile(csv_file_name_path):
        with open(csv_file_name_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['gse', 'gsm','char_tissue_key','char_tissue',
                                                    "tissue_match_key",'tissue_key_bank','tissue_key_word'] 
            ...
            if existed_GSE_choose :
                try:
                    ...
                    if  gse in gse_list:
                        ...
                        GSE_match_key,GSE_key_bank,GSE_key_word=find_key_words(GSE_meta_dict,["cancer_celllines_bank","tumor_tissues_bank","tissue_list_bank"],[cancer_celllines_bank,tumor_tissues_bank,tissue_list_bank])
                        with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        fieldnames = ['gse', 'gsm', 'char_tissue_key',
                                    'char_tissue',"tissue_match_key",'tissue_key_bank','tissue_key_word']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gse, 
                                                                                        'char_tissue_key':'','char_tissue':'',
                                                                                        'tissue_match_key':GSE_match_key,'tissue_key_bank':GSE_key_bank,'tissue_key_word':GSE_key_word
                                                                                        })
                        print("{} is starting ".format(gse))
                        if not only_need and len(files) > 1:
                            for file in files:
                                if file.startswith('GSM') and file.endswith('.xml'):
                                    gsm,char_tissue_key,char_tissue,tissue_match_key,tissue_key_bank,tissue_key_word = find_all_meta(folder_path,file,Tissue_key_list,["cancer_celllines_bank","tumor_tissues_bank","tissue_list_bank"],[cancer_celllines_bank,tumor_tissues_bank,tissue_list_bank])
                                    #if char_tissue_key != '' or char_tissue != '' or tissue_match_key != [] or tissue_key_bank != [] or tissue_key_word != []:
                                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                            fieldnames = ['gse', 'gsm', 'char_tissue_key',
                                        'char_tissue',"tissue_match_key",'tissue_key_bank','tissue_key_word']                            
                                            writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                            writer.writerow({'gse': gse, 'gsm': gsm, 
                                                                                            'char_tissue_key':char_tissue_key,'char_tissue':char_tissue,
                                                                                            'tissue_match_key':tissue_match_key,'tissue_key_bank':tissue_key_bank,'tissue_key_word':tissue_key_word
                                                                                            })
