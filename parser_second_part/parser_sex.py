...
def find_characteristics_key(dict,key_list):
    ...
    sex_key_list_2 = ['sex','gender','female','male','woman','women','man','men']
    characteristics_need={}
    for key in keys_to_check:
        for characteristics_key,characteristics_value in characteristics_dict.items():
            if key in characteristics_key:
                characteristics_need[characteristics_key] = characteristics_dict[characteristics_key]
            else:
                for key_2 in sex_key_list_2: 
                    if re.search(rf'\b{re.escape(key_2)}\b', characteristics_value):
                        characteristics_need[characteristics_key] = characteristics_dict[characteristics_key]                 
...
def find_all_meta(folder_path,file,key_list):
    if file.startswith('GSM') and file.endswith('.xml'):
                ...
                #match_key,key_bank,key_word=find_key_words(GSE_GSM_dict,word_bank_name_list,word_bank_list)
...
def main():
    ...
    csv_file_name = "sex_"+ args.all_meta_file_path
    no_meta_file_name = "sex_"+ args.no_meta_file_path
    error_file_name  = "sex_"+ args.error_file_path
...
##char_key_list
    ...
    sex_key_list = ['sex','gender','male']
    sex_bank = ['sex','gender','male']
    ...
    if save_path == '':
        if not os.path.exists('sex'):
            os.makedirs("sex")
            csv_file_name_path = os.path.join("sex", csv_file_name)
            no_meta_file_name_path = os.path.join("sex", no_meta_file_name)
            error_file_name_path = os.path.join("sex", error_file_name)
    else:
        if not os.path.exists(os.path.join(save_path,"sex")):
            os.makedirs(os.path.join(save_path,"sex"))
        csv_file_name_path = os.path.join(save_path,"sex", csv_file_name)
        no_meta_file_name_path =os.path.join(save_path,"sex",  no_meta_file_name)
        error_file_name_path = os.path.join(save_path,"sex",  error_file_name)
    if not os.path.isfile(csv_file_name_path):
        with open(csv_file_name_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['gse', 'gsm','char_sex_key','char_sex'
                                                   ]
                    ...
                    if  gse in gse_list :
                        ...
                        with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        ...
                                        writer.writerow({'gse': gse, 'gsm': gse, 
                                                                                        'char_sex_key':'','char_sex':''
                                                                                        
                                                                                        })
                        if not only_need and len(files) > 1: 
                            for file in files:
                                if file.startswith('GSM') and file.endswith('.xml'):
                                    gsm,char_sex_key,char_sex = find_all_meta(folder_path,file,sex_key_list)
                                    #if char_sex_key != '' or char_sex != '' :
                                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                            fieldnames = ['gse', 'gsm', 'char_sex_key','char_sex']                            
                                            writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                            writer.writerow({'gse': gse, 'gsm': gsm, 
                                                                                            'char_sex_key':char_sex_key,'char_sex':char_sex
                    ...
                    if not only_need and len(files) > 1: 
                        for file in files:
                            if file.startswith('GSM') and file.endswith('.xml'):
                                ...
                                #if char_sex_key != '' or char_sex != '' :
                                with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        fieldnames = ['gse', 'gsm', 'char_sex_key',
                                    'char_sex']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gsm, 
                                                                                        'char_sex_key':char_sex_key,'char_sex':char_sex
