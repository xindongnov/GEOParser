...
def main():
    ...
    csv_file_name = "race_"+ args.all_meta_file_path
    no_meta_file_name = "race_"+ args.no_meta_file_path
    error_file_name  = "race_"+ args.error_file_path
...
##char_key_list
    ...
    Race_key_list = ["race","strain","background"]
    race_bank = ["race","strain","background"]
    ...
    if save_path == '':
        if not os.path.exists('race'):
            os.makedirs("race")
            csv_file_name_path = os.path.join("race", csv_file_name)
            no_meta_file_name_path = os.path.join("race", no_meta_file_name)
            error_file_name_path = os.path.join("race", error_file_name)
    else:
        if not os.path.exists(os.path.join(save_path,"race")):
            os.makedirs(os.path.join(save_path,"race"))
        csv_file_name_path = os.path.join(save_path,"race", csv_file_name)
        no_meta_file_name_path =os.path.join(save_path,"race",  no_meta_file_name)
        error_file_name_path = os.path.join(save_path,"race",  error_file_name)
    if not os.path.isfile(csv_file_name_path):
        with open(csv_file_name_path, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['gse', 'gsm','char_race_key','char_race'
                                                   ] 
                ...
                try:
                    ...
                    if  gse in gse_list :
                        ...
                        with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                        fieldnames = ['gse', 'gsm', 'char_race_key',
                                    'char_race']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gse, 
                                                                                        'char_race_key':'','char_race':''
                                                                                        
                                                                                        })
                        if not only_need and len(files) > 1:    
                            for file in files:
                                if file.startswith('GSM') and file.endswith('.xml'):
                                    gsm,char_race_key,char_race = find_all_meta(folder_path,file,Race_key_list)
                                    #if char_race_key != '' or char_race != '' :
                                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                            fieldnames = ['gse', 'gsm', 'char_race_key',
                                        'char_race']                            
                                            writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                            writer.writerow({'gse': gse, 'gsm': gsm, 
                                                                                            'char_race_key':char_race_key,'char_race':char_race
                    ...
                    with open(csv_file_name_path, 'a', newline='', encoding='utf-8') as csvfile:
                                    ...
                                    'char_race']                            
                                        writer = csv.DictWriter(csvfile,fieldnames=fieldnames)
                                        writer.writerow({'gse': gse, 'gsm': gsm, 'char_race_key':'','char_race':''})
            else:
                try:
                        GSE_file = [file for file in files if file.startswith('GSE')]
                        #xml_file_path = os.path.join(folder_path, GSE_file[0])
